...
plan_set_operations(PlannerInfo *root)
{
	...
	/*modified by jim in 2022.7.1*/
	if (root->search_plan_mode == 0){
		add_path(setop_rel, path);
	}else{
        add_path_explore(setop_rel, path, root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
	}
	...
	/*choose corresponding set function, modified by jim in 2022.7.1*/
	if(root->search_plan_mode==0){
		set_cheapest(setop_rel);
	}else
	{
		/* Find and save the cheapest paths for this rel */
		set_cheapest_explore(setop_rel);
	}
