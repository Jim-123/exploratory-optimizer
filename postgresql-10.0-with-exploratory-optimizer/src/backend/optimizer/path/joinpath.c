...
static void sort_inner_and_outer_origin(PlannerInfo *root, RelOptInfo *joinrel,
					 RelOptInfo *outerrel, RelOptInfo *innerrel,
					 JoinType jointype, JoinPathExtraData *extra);
static void match_unsorted_outer(PlannerInfo *root, RelOptInfo *joinrel,
					 RelOptInfo *outerrel, RelOptInfo *innerrel,
					 JoinType jointype, JoinPathExtraData *extra);
static void match_unsorted_outer_origin(PlannerInfo *root, RelOptInfo *joinrel,
					 RelOptInfo *outerrel, RelOptInfo *innerrel,
					 JoinType jointype, JoinPathExtraData *extra);
...
static void hash_inner_and_outer(PlannerInfo *root, RelOptInfo *joinrel,
					 RelOptInfo *outerrel, RelOptInfo *innerrel,
					 JoinType jointype, JoinPathExtraData *extra);
static void hash_inner_and_outer_origin(PlannerInfo *root, RelOptInfo *joinrel,
					 RelOptInfo *outerrel, RelOptInfo *innerrel,
					 JoinType jointype, JoinPathExtraData *extra);
...
static void generate_mergejoin_paths(PlannerInfo *root,
						 ...
						 List *merge_pathkeys,
						 bool is_partial);
/* explore written by jim */
static void generate_mergejoin_paths_explore(PlannerInfo *root,
						 RelOptInfo *joinrel,
						 RelOptInfo *innerrel,
						 Path *outerpath,
						 JoinType jointype,
					 ...
					 List *restrictlist)
{
	...
	if (mergejoin_allowed){
	    if (root->search_plan_mode==0){
			sort_inner_and_outer_origin(root, joinrel, outerrel, innerrel,
								 jointype, &extra);
	...
	if (mergejoin_allowed){
		if (root->search_plan_mode==0){
			match_unsorted_outer_origin(root, joinrel, outerrel, innerrel,
							 jointype, &extra);
	...
	if (enable_hashjoin || jointype == JOIN_FULL){
		if (root->search_plan_mode==0){
			hash_inner_and_outer_origin(root, joinrel, outerrel, innerrel,
							 jointype, &extra);
		}else
		{
				  ...
				  JoinPathExtraData *extra)
{
	...
	}else
	{
		...
		if (add_path_precheck_explore(joinrel,
							workspace.startup_cost, workspace.total_cost,
							pathkeys, required_outer, workspace.explore_value))
		{
			add_path_explore(joinrel, (Path *)
					create_nestloop_path(root,
										joinrel,
										jointype,
										&workspace,
										...
										extra->restrictlist,
										pathkeys,
										required_outer), root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
		}
		else
		{
			/* Waste no memory when we reject a path here */
    ...
    if (root->search_plan_mode==0){
		...
		*/
		initial_cost_mergejoin(root, &workspace, jointype, mergeclauses,
							...
							extra);
		/* we use add_path_precheck_explore instead of add_path_precheck, written by jim */
		if (add_path_precheck(joinrel,
							workspace.startup_cost, workspace.total_cost,
							pathkeys, required_outer))
		{
			/* we use add_path_explore instead of add_path_precheck, written by jim */
			add_path(joinrel, (Path *)
							...
							pathkeys, required_outer, workspace.explore_value))
		{
			...
			add_path_explore(joinrel, (Path *)
					create_mergejoin_path(root,
										...
										mergeclauses,
										outersortkeys,
										innersortkeys), root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
		}
		else
		{
			/* Waste no memory when we reject a path here */
				  ...
				  JoinPathExtraData *extra)
{
	...
	}else
	{
		...
		if (add_path_precheck_explore(joinrel,
								workspace.startup_cost, workspace.total_cost,
								NIL, required_outer, workspace.explore_value))
		{
			add_path_explore(joinrel, (Path *)
						create_hashjoin_path(root,
											joinrel,
											...
											extra->restrictlist,
											required_outer,
											hashclauses), root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
		}
		else
		{
			/* Waste no memory when we reject a path here */
					 ...
					 JoinPathExtraData *extra)
{
    ...
    /* prepare for parallel inner path, written by jim */
	
	inner_path_parallel = innerrel->cheapest_total_path;
	/*
	...
	*/
	...
	{
		inner_path_parallel = (Path *) create_unique_path(root, innerrel,
												inner_path_parallel, extra->sjinfo);
		Assert(inner_path_parallel);
		jointype = JOIN_INNER;
	}
	/*
	...
	*/
		...
		bms_is_empty(joinrel->lateral_relids))
	{
		...
		if (inner_path_parallel->parallel_safe)
			cheapest_safe_inner = inner_path_parallel;
	...
	*/
	...
	foreach(l, all_pathkeys)
	{
		...
		/*
		* written by jim, try cheapest_cost_explore_pathlist instead of cheapest cost path(origin method)
		*/
		foreach(lc1, outerrel->cheapest_cost_explore_pathlist)
		{
			outer_path = (Path *) lfirst(lc1);
			foreach(lc2, innerrel->cheapest_cost_explore_pathlist)
			{
				inner_path = (Path *) lfirst(lc2);
				/*
...
static void
sort_inner_and_outer_origin(PlannerInfo *root,
					 RelOptInfo *joinrel,
					 RelOptInfo *outerrel,
					 RelOptInfo *innerrel,
					 JoinType jointype,
...
/*
 * generate_mergejoin_paths_explore (modified by jim)
 *	Creates possible mergejoin paths for input outerpath.
 *
 * We generate mergejoins if mergejoin clauses are available.  We have
 * two ways to generate the inner path for a mergejoin: sort the cheapest
 ...
 */
static void
generate_mergejoin_paths_explore(PlannerInfo *root,
						 RelOptInfo *joinrel,
						 RelOptInfo *innerrel,
						 Path *outerpath,
						 JoinType jointype,
						 ...
						 bool is_partial)
{
	...
	innersortkeys = make_inner_pathkeys_for_merge(root,
												  mergeclauses,
												  outerpath->pathkeys);
	/* try each path in cheapest_totals, written by jim */
    foreach(p1, inner_cheapest_totals)
	{
		Path	   *inner_cheapest_total = (Path *) lfirst(p1);
		/*
		* Generate a mergejoin on the basis of sorting the cheapest inner. Since
		* a sort will be needed, only cheapest total cost matters. (But
		* try_mergejoin_path will do the right thing if inner_cheapest_total is
	...
	/*Find cheapest total/start plan, modified by jim */
	cheapest_startup_inner = NULL;
	cheapest_total_inner = NULL;
	foreach(p1, inner_cheapest_totals)
	{
		...
		{
			/* set startup inner */
			if(cheapest_startup_inner==NULL || compare_path_costs(inner_cheapest_total,cheapest_startup_inner,STARTUP_COST) < 0)
			{
				cheapest_startup_inner = inner_cheapest_total;
			}
			/* set total inner */
			if(cheapest_total_inner==NULL || compare_path_costs(inner_cheapest_total,cheapest_total_inner,TOTAL_COST) < 0)
			{
				cheapest_total_inner = inner_cheapest_total;
			}
		}
					 ...
					 JoinPathExtraData *extra)
{
	...
	Path	   *inner_cheapest_total = NULL;
	Path	   *matpath = NULL;
	Path	   *outerpath = NULL;
	ListCell   *lc1;
    ListCell   *lc11;
    List       *inner_paths_explore_modify = NIL;
	List       *inner_paths_explore_mat = NIL;
	List       *inner_cheapest_totals = NIL;
	/*
	...
	/*try each plan in cheapest_cost_explore_pathlist  modified by jim */
	foreach(lc1, innerrel->cheapest_cost_explore_pathlist)
	{
		inner_cheapest_total = (Path *) lfirst(lc1);
		/*
		...
		*/
		if (save_jointype == JOIN_UNIQUE_INNER)
		{
			...
			/*add explore value path */
			inner_paths_explore_modify = lappend(inner_paths_explore_modify, inner_cheapest_total);
		}
		else if (nestjoinOK)
		{
			/*
			...
			*/
				...
				!ExecMaterializesOutput(inner_cheapest_total->pathtype))
			{
				...
				/*add explore value path */
				inner_paths_explore_mat = lappend(inner_paths_explore_mat, matpath);
			}
		}
		/* also add nomal to inner_cheapest_totals */
		if (inner_cheapest_total != NULL)
		{
		    inner_cheapest_totals = lappend(inner_cheapest_totals, inner_cheapest_total);
		}
	}
	/* For JOIN_UNIQUE_INNER, if inner_paths_explore_modify is null, then return*/
	if (save_jointype == JOIN_UNIQUE_INNER)
	{
		if (inner_paths_explore_modify==NIL)
		{
			return;
		}
	}
    /*Try each explore plan in outerrel */
	foreach(lc1, outerrel->pathlist)
	{
		List	   *merge_pathkeys;
		outerpath = (Path *) lfirst(lc1);
		/*
		...
		{
			//modified by jim (origin:(outerpath != outerrel->cheapest_total_path))
			if (!is_include_in_pathlist(outerpath, outerrel->cheapest_cost_explore_pathlist))
				continue;
		...
		if (save_jointype == JOIN_UNIQUE_INNER)
		{
			/*try each path in inner_paths_explore_modify */
			foreach(lc11, inner_paths_explore_modify)
			{
				Path	   *inner_path = (Path *) lfirst(lc11);
				/*
		...
		else if (nestjoinOK)
		{
			...
			if (inner_paths_explore_mat != NIL)
			{
				foreach(lc3, inner_paths_explore_mat)
				{
					Path	   *innerpath = (Path *) lfirst(lc3);
		...
		if (inner_cheapest_totals == NIL)
			continue;

		/* Generate merge join paths */
		generate_mergejoin_paths_explore(root, joinrel, innerrel, outerpath,
								 save_jointype, extra, useallclauses,
								 inner_cheapest_totals, merge_pathkeys,
								 false);
	}
	/*
...
static void
match_unsorted_outer_origin(PlannerInfo *root,
					 RelOptInfo *joinrel,
					 RelOptInfo *outerrel,
					 RelOptInfo *innerrel,
					 JoinType jointype,
					 ...
					 JoinPathExtraData *extra)
{
	...
	if (hashclauses)
	{
		...
		Path       *cheapest_startup_outer = outerrel->cheapest_startup_path;
		int         flag = 0; /* 0: no tried; 1:have tried */
		/* we also try path of the cheapest start cost */
		foreach(p1, outerrel->cheapest_cost_explore_pathlist)
		{
			cheapest_total_outer = (Path *) lfirst(p1);
			if(cheapest_total_outer == cheapest_startup_outer)
			{
				flag = 1;
			}
			foreach(p2, innerrel->cheapest_cost_explore_pathlist)
			{
				cheapest_total_inner = (Path *) lfirst(p2);
				/*
				* If either cheapest-total path is parameterized by the other rel, we
		...
		if (flag == 0 && cheapest_startup_outer != NULL)
		{
			if(jointype != JOIN_UNIQUE_OUTER)
			{
				if (jointype == JOIN_UNIQUE_INNER)
				{
					/* try cheapest_startup_outer*/
					foreach(p3, innerrel->cheapest_cost_explore_pathlist)
					{
						cheapest_total_inner = (Path *) lfirst(p3);
						cheapest_total_inner = (Path *)
							create_unique_path(root, innerrel,
											cheapest_total_inner, extra->sjinfo);
						Assert(cheapest_total_inner);
				...
				{
					/* try cheapest_startup_outer*/
					foreach(p4, innerrel->cheapest_cost_explore_pathlist)
					{
						cheapest_total_inner = (Path *) lfirst(p4);
						try_hashjoin_path(root,
										joinrel,
										cheapest_startup_outer,
										cheapest_total_inner,
		...
		/* when jointype is nomal, we also need try each path in inner/outter parameterized paths, moved by jim */
		if (jointype != JOIN_UNIQUE_OUTER && jointype != JOIN_UNIQUE_INNER )
		{
			ListCell   *lc1;
			ListCell   *lc2;
				...
				foreach(lc2, innerrel->cheapest_parameterized_paths)
				{
					...
					*/
					if (PATH_PARAM_BY_REL(innerpath, outerrel))
						continue;
					// /* need romove path we have tried*/
					// if (outerpath == cheapest_startup_outer &&
					// 	innerpath == cheapest_total_inner)
					// 	continue;	/* already tried it */
					/* need romove path we have tried, rewritten by jim*/
					if ((is_include_in_pathlist(outerpath, outerrel->cheapest_cost_explore_pathlist) &&
						is_include_in_pathlist(innerpath, innerrel->cheapest_cost_explore_pathlist))||
						(outerpath == cheapest_startup_outer && is_include_in_pathlist(innerpath, innerrel->cheapest_cost_explore_pathlist)))
						continue;	/* already tried it */
					try_hashjoin_path(root,
									joinrel,
									outerpath,
...
static void
hash_inner_and_outer_origin(PlannerInfo *root,
					 RelOptInfo *joinrel,
					 RelOptInfo *outerrel,
					 RelOptInfo *innerrel,
					 JoinType jointype,
