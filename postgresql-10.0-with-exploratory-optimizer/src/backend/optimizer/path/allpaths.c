...
make_one_rel(PlannerInfo *root, List *joinlist)
{
	...
	/* we record the levels_needed into root in order to calculate the represent of the current query in join_size_estimation
	 * modified by jim 2021.3.11
	*/
    root->levels_needed = list_length(joinlist);
...
set_rel_pathlist(PlannerInfo *root, RelOptInfo *rel,
				 Index rti, RangeTblEntry *rte)
{
	// move from set_base_rel_pathlists by jim 2021.1.27
...
set_plain_rel_pathlist(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte)
{
	...
	//modified by jim 2021.1.27
	if (root->search_plan_mode == 0){
		add_path(rel, create_seqscan_path(root, rel, required_outer, 0));
	}else
	{
		/* code */
		add_path_explore(rel, create_seqscan_path(root, rel, required_outer, 0), root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
	}
