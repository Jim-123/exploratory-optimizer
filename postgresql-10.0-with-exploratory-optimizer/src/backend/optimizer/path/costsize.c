			 ...
			 RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	if (param_info){
		path->rows = param_info->ppi_rows;
		//jim 2021.1.27
		path->explore_value = param_info->ppi_explore_value;
	}else{
        path->rows = baserel->rows;
		//jim 2021.1.27
		path->explore_value = baserel->explore_value;
	}
				  ...
				  double *rows)
{
	...
	if (rows){
		...
		path->path.rows = param_info->ppi_rows;
		//jim 2021.1.27
		path->path.explore_value = param_info->ppi_explore_value;
	}else{
		path->path.rows = rel->rows;
		//jim 2021.1.27
		path->path.explore_value = rel->explore_value;
	}
		   ...
		   bool partial_path)
{
	...
	if (path->path.param_info)
	{
		...
		//jim 2021.1.27
		path->path.explore_value = path->path.param_info->ppi_explore_value;
	...
	else
	{
		...
		//jim 2021.1.27
		path->path.explore_value = baserel->explore_value;
...
cost_recursive_union(Path *runion, Path *nrterm, Path *rterm)
{
	...
	runion->explore_value = nrterm->explore_value + rterm->explore_value;
}
					  ...
					  JoinPathExtraData *extra)
{
	...
	/* Save explore value */
	workspace->explore_value = outer_path->explore_value + inner_path->explore_value;
}
void
initial_cost_nestloop_explore(PlannerInfo *root, JoinCostWorkspace *workspace,
					  JoinType jointype,
					  Path *outer_path, Path *inner_path,
					  JoinPathExtraData *extra, RelOptInfo *joinrel)
{
	Cost		startup_cost = 0;
	Cost		run_cost = 0;
	double		outer_path_rows = outer_path->rows;
	...
	workspace->run_cost = run_cost;
	/* Save explore value */
	workspace->explore_value = joinrel->explore_value + outer_path->explore_value + inner_path->explore_value;
}
					...
					JoinPathExtraData *extra)
{
	...
	//modified by jim 2021.1.29
	if (path->path.param_info){
		path->path.rows = path->path.param_info->ppi_rows;
		//also explore value
		path->path.explore_value = path->path.param_info->ppi_explore_value + workspace->explore_value;
	}else{
        path->path.rows = path->path.parent->rows;
	    path->path.explore_value = path->path.parent->explore_value + workspace->explore_value;
	}
	...
	path->path.startup_cost = startup_cost;
	path->path.total_cost = startup_cost + run_cost;
	/* also save explore value */
	// path->path.explore_value = workspace->explore_value;
}
					   ...
					   JoinPathExtraData *extra)
{
	...
	/* Calculate explore value  written by jim */
	workspace->explore_value = outer_path->explore_value + inner_path->explore_value;
...
void
initial_cost_mergejoin_explore(PlannerInfo *root, JoinCostWorkspace *workspace,
					   JoinType jointype,
					   List *mergeclauses,
					   Path *outer_path, Path *inner_path,
					   List *outersortkeys, List *innersortkeys,
					 ...
					 JoinPathExtraData *extra)
{
	...
	if (path->jpath.path.param_info){
		...
		path->jpath.path.explore_value = path->jpath.path.param_info->ppi_explore_value + workspace->explore_value;
	}else{
		path->jpath.path.rows = path->jpath.path.parent->rows;
		path->jpath.path.explore_value = path->jpath.path.parent->explore_value + workspace->explore_value;
	}
	...
	/*explore value written by jim */
	// path->jpath.path.explore_value = workspace->explore_value;
}
					  ...
					  JoinPathExtraData *extra)
{
	...
	/* Save explore value */
	workspace->explore_value = inner_path->explore_value + outer_path->explore_value;
}

void
initial_cost_hashjoin_explore(PlannerInfo *root, JoinCostWorkspace *workspace,
					  JoinType jointype,
					  List *hashclauses,
					  Path *outer_path, Path *inner_path,
					  JoinPathExtraData *extra, RelOptInfo *joinrel)
{
	...
	workspace->numbatches = numbatches;
	/* Save explore value */
	workspace->explore_value = joinrel->explore_value + inner_path->explore_value + outer_path->explore_value;
}
/*
 * final_cost_hashjoin
					...
					JoinPathExtraData *extra)
{
	...
	path->jpath.path.startup_cost = startup_cost;
	path->jpath.path.total_cost = startup_cost + run_cost;
	/* Add explore value to path, by jim */
	//path->jpath.path.explore_value = workspace->explore_value;
}
