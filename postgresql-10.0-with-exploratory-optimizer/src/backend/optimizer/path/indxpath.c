...
create_index_paths(PlannerInfo *root, RelOptInfo *rel)
{
	...
	if (bitindexpaths != NIL)
	{
		...
		if (root->search_plan_mode == 0){
            ...
            add_path_explore(rel, (Path *) bpath, root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
		}
				...
				List **bitindexpaths)
{
	...
	foreach(lc, indexpaths)
	{
		...
		if (index->amhasgettuple){
			...
			{
				/* we neeed to consider explore value*/
				add_path_explore(rel, (Path *) ipath, root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
			}
