...
/*
 * set_cheapest_explore
 *	  Find the minimum-cost and explore value paths from among a relation's paths,
 *	  and save them in the rel's pathlist fields.
 *    written by jim
 */
void
set_cheapest_explore(RelOptInfo *parent_rel)
{
	Path	   *cheapest_startup_path;
	Path	   *cheapest_total_path;
	Path	   *best_param_path;
	List	   *parameterized_paths;
	List       *cheapest_cost_explore_paths;/* cheapest path balance between cost and explore value */
	ListCell   *p;
	...
	cheapest_startup_path = cheapest_total_path = best_param_path = NULL;
	parameterized_paths = NIL;
	cheapest_cost_explore_paths = NIL;
	foreach(p, parent_rel->pathlist)
	{
		Path	   *path = (Path *) lfirst(p);
		int			cmp;
		...
		else
		{
			...
			if (cheapest_total_path == NULL)
			{
				cheapest_startup_path = cheapest_total_path = path;
				/* by jim */
				cheapest_cost_explore_paths = lappend(cheapest_cost_explore_paths, path);
				continue;
			}
			...
			/* set the pathlist cheapest_cost_explore_paths which calculated by total cost and explore value, written by jim*/
			if(cheapest_cost_explore_paths == NIL)
			{
				cheapest_cost_explore_paths = lappend(cheapest_cost_explore_paths, path);
			}
			else
			{
				...
				Path *oldpath;
				bool addnew = true;
				for(p1 = list_head(cheapest_cost_explore_paths); p1 != NULL; p1 = p1_next)
				{
					p1_next = lnext(p1);
					oldpath = (Path *) lfirst(p1);
					bool remove_old = false;
					if(compare_path_costs(path, oldpath, TOTAL_COST)>0)
					{
						if(path->explore_value <= oldpath->explore_value)
						{
							/*no need to add, return false directly*/
							addnew = false;
							break;
						}
						
					}else if(compare_path_costs(path, oldpath, TOTAL_COST)==0)
					{
						if(path->explore_value < oldpath->explore_value)
						{
							/*no need to add, return false directly*/
					...
					/*delete old path if true */
					/*
					* Remove current element from pathlist if dominated by new.
					*/
					if (remove_old)
					{
						cheapest_cost_explore_paths = list_delete_cell(cheapest_cost_explore_paths,
																p1, p1_prev);
	...
	if (cheapest_cost_explore_paths == NIL)
		cheapest_cost_explore_paths = lappend(cheapest_cost_explore_paths, best_param_path);
	Assert(cheapest_cost_explore_paths != NIL);

	parent_rel->cheapest_startup_path = cheapest_startup_path;
	parent_rel->cheapest_total_path = cheapest_total_path;
	parent_rel->cheapest_unique_path = NULL;	/* computed only if needed */
	parent_rel->cheapest_parameterized_paths = parameterized_paths;
	parent_rel->cheapest_cost_explore_pathlist = cheapest_cost_explore_paths; 
}
 ...
 */
static int
compare_path_explore_value_fuzzily(Path *path1, Path *path2, double fuzz_factor)
{
	if (path1->explore_value > path2->explore_value * fuzz_factor)
	{
		return 1;
	}
	if (path2->explore_value > path1->explore_value * fuzz_factor)
	{
		return -1;
	}
	/* fuzzily the same on both costs */
...
void
add_path_explore(RelOptInfo *parent_rel, Path *new_path, Cost best_pred_cost_of_query, double prune_rate)
{
	bool		accept_new = true;	/* unless we find a superior old path */
	ListCell   *insert_after = NULL;	/* where to insert new item */
	List	   *new_path_pathkeys;
	...
	ListCell   *p1_prev;
	ListCell   *p1_next;
	int         need_continue = 0;   /*add by jim in 2022.7.4*/
	double      best_cost = 0;
	double      ev_fuzzy_r = 1.1;
	double      ev_fuzzy_r2 = 1.01;
	best_cost = parent_rel->best_cost;
	/*
	 * This is a convenient place to check for query cancel --- no part of the
	 * planner goes very long without calling add_path().
	 */
	...
	/*add by jim in 2022.7.4, judge whether the cost of new_path is larger than the cost of best plan of this query,
	if yes, prune it directly, else, go on to check*/
	if(best_pred_cost_of_query == 0||parent_rel->pathlist == NIL||new_path->total_cost<best_cost){
		need_continue = 1;
		if(parent_rel->pathlist == NIL||new_path->total_cost<best_cost){
			parent_rel->best_cost = new_path->total_cost;
		}
	}else if (best_pred_cost_of_query > 0){
		if(new_path->total_cost <= best_pred_cost_of_query*prune_rate){
			need_continue = 1;
		}else{
			need_continue = 0;
			accept_new = false;
	...
	if(need_continue == 1){
		...
		for (p1 = list_head(parent_rel->pathlist); p1 != NULL; p1 = p1_next)
		{
			...
			*/
			costcmp = compare_path_costs_fuzzily(new_path, old_path,
												STD_FUZZ_FACTOR);
			evcmp = compare_path_explore_value_fuzzily(new_path, old_path, ev_fuzzy_r);
				...
				if (keyscmp != PATHKEYS_DIFFERENT)
				{
					switch (costcmp)
					{
						case COSTS_EQUAL:
										...
										*/
										if (new_path->parallel_safe >
											...
											remove_old = true;	/* new dominates old */
																			...
																			old_path,
																			1.0000000001) == COSTS_BETTER2)
											accept_new = false; /* old equals or
																* dominates new */
										else if (compare_path_explore_value_fuzzily(new_path, old_path, ev_fuzzy_r2) == 1)
											remove_old = true;
										else
											accept_new = false;
									}
							...
							{
								//when new explore value better than old
								if (keyscmp != PATHKEYS_BETTER2)
								{
									if ((outercmp == BMS_EQUAL ||
										outercmp == BMS_SUBSET1) &&
										new_path->rows <= old_path->rows &&
										new_path->parallel_safe >= old_path->parallel_safe)
										//2021.12.7, we need compare new_path and oldpath's cost exactly for preventing remove best cost plan
										if(compare_path_costs_fuzzily(new_path,
																			old_path,
																			1.00009) != COSTS_BETTER2){
											remove_old = true;	/* new dominates old */
										}
								}
							}
							...
							{
								// when old explore value better than new
								if (keyscmp != PATHKEYS_BETTER1)
								{
									if ((outercmp == BMS_EQUAL ||
										outercmp == BMS_SUBSET2) &&
...
// void
// add_path_explore(RelOptInfo *parent_rel, Path *new_path)
// {
// 	bool		accept_new = true;	/* unless we find a superior old path */
// 	ListCell   *insert_after = NULL;	/* where to insert new item */
// 	List	   *new_path_pathkeys;
...
// 								new_path->rows <= old_path->rows &&
// 								new_path->parallel_safe >= old_path->parallel_safe&&
// 								new_path->explore_value>=old_path->explore_value)
// 								if(compare_path_costs_fuzzily(new_path,
// 																		old_path,
// 																		1.0001) != COSTS_BETTER2)
// 									remove_old = true;	/* new dominates old */
// 						}
// 						else if (keyscmp == PATHKEYS_BETTER2)
// 						{
...
// 								 * and total costs compare differently.
// 								 */
// 								if(new_path->explore_value>old_path->explore_value)
// 									remove_old = true;
// 								else if (new_path->explore_value<old_path->explore_value)
// 									accept_new = false;
// 								else if (new_path->parallel_safe >
// 									old_path->parallel_safe)
// 									remove_old = true;	/* new dominates old */
...
// }
/**
 * try trypath in pathlist
 */
bool is_include_in_pathlist(Path *trypath, List *pathlist)
{
	ListCell *p1;
	bool result = false;
	foreach(p1, pathlist)
	{
		Path *currentPath = (Path *) lfirst(p1);
		if (trypath == currentPath){
			result = true;
			return result;
		}
	}
...
*/
bool
add_path_precheck_explore(RelOptInfo *parent_rel,
				  Cost startup_cost, Cost total_cost,
				  List *pathkeys, Relids required_outer, double explore_value)
{
	List	   *new_path_pathkeys;
	bool		consider_startup;
	ListCell   *p1;
	...
	foreach(p1, parent_rel->pathlist)
	{
		...
		if (total_cost > old_path->total_cost * STD_FUZZ_FACTOR)
		{
			...
			if (startup_cost > old_path->startup_cost * STD_FUZZ_FACTOR ||
				!consider_startup)
			{
				/*new path explore_value less than old path */
				if (explore_value <= old_path->explore_value)
				{
					/* new path loses on cost, so check pathkeys... */
					List	   *old_path_pathkeys;
				   ...
				   int parallel_workers, List *partitioned_rels)
{
	...
	foreach(l, subpaths)
	{
		...
		pathnode->path.explore_value += subpath->explore_value; /*add by jim in 2022.7.1*/
...
create_material_path(RelOptInfo *rel, Path *subpath)
{
	...
	//also copy the explore_value, written by jim
	pathnode->path.explore_value = subpath->explore_value;
					   ...
					   PathTarget *target)
{
		...
		equal(oldtarget->exprs, target->exprs))
	{
		...
		/*add explore value, written by jim*/
		pathnode->path.explore_value = subpath->explore_value;
	}
	else
	{
						   ...
						   PathTarget *target)
{
	...
	/*copy explore value, written by jim*/
	pathnode->path.explore_value = subpath->explore_value;
	return pathnode;
}
						...
						int epqParam)
{
	...
	foreach(lc, subpaths)
	{
		...
		//modified by jim, also calculate the explore value
