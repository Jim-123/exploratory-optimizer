...
set_estimated_cost_hook_type estimated_cost_hook = NULL;
...
standard_planner(Query *parse, int cursorOptions, ParamListInfo boundParams)
{
	...
	/* choose the corresponding mode to generate best path, modified by jim*/
	if (root->search_plan_mode==0){
		best_path = get_cheapest_fractional_path(final_rel, tuple_fraction);
	}else
	{
		best_path = get_cheapest_explore_fractional_path(final_rel, tuple_fraction, root->rate_to_generate_explore_plan);
	}
	//we need a hook to record best_path's estimated cost.
	if (estimated_cost_hook)
		(*estimated_cost_hook)(best_path);
				 ...
				 bool hasRecursion, double tuple_fraction)
{
	...
	}else
	{
		...
		set_cheapest_explore(final_rel);
	}
				 ...
				 double tuple_fraction)
{
	...
	else
	{
		...
		/*if we use our model, we need also apply*/
		if(root->search_plan_mode > 1){
			/* also apply to cheapest_cost_explore_paths, written by jim */
			foreach(lc, current_rel->cheapest_cost_explore_pathlist)
			{
				Path	   *subpath = (Path *) lfirst(lc);
				Path	   *path;
	...
	foreach(lc, current_rel->pathlist)
	{
		...
		{
			/* cardlearner or our method*/
			add_path_explore(final_rel, path, root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
		}
					  ...
					  grouping_sets_data *gd)
{
	...
	if (can_sort)
	{
		...
		foreach(lc, input_rel->pathlist)
		{
			...
			is_sorted = pathkeys_contained_in(root->group_pathkeys,
											  path->pathkeys);
			/*modified by jim (origin path == cheapest_path)*/
			if (root->search_plan_mode == 0){
			...
			{
				if (is_include_in_pathlist(path, input_rel->cheapest_cost_explore_pathlist)|| is_sorted)
				{
					/* Sort the cheapest-total path if it isn't already sorted */
					if (!is_sorted)
						path = (Path *) create_sort_path(root,
					...
					else if (parse->hasAggs)
					{
						...
						*/
						...
						add_path_explore(grouped_rel, (Path *)
								create_agg_path(root,
												grouped_rel,
												path,
												target,
												...
												(List *) parse->havingQual,
												agg_costs,
												dNumGroups), root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
	...
	if (can_hash)
	{
		//modified by jim , we not only consider the cheapest cost path, but also the plan in cheapest_cost_explore_path
		//original: just consider the cheapestpath
		if (root->search_plan_mode == 0){
			if (parse->groupingSets)
			{
				/*
		...
		{
			foreach(lc, input_rel->cheapest_cost_explore_pathlist)
			{
				Path	   *cheapest_path_explore = (Path *) lfirst(lc);
				if (parse->groupingSets)
				{
					/*
					* Try for a hash-only groupingsets path over unsorted input.
					*/
					consider_groupingsets_paths(root, grouped_rel,
												cheapest_path_explore, false, true, target,
												gd, agg_costs, dNumGroups);
				}
				else
				{
					hashaggtablesize = estimate_hashagg_tablesize(cheapest_path_explore,
																agg_costs,
																dNumGroups);
	...
	{
		/* Find and save the cheapest paths for this rel */
		set_cheapest_explore(grouped_rel);
	}
					...
					List *activeWindows)
{
	...
	foreach(lc, input_rel->pathlist)
	{
		...
		{
			/*modified by jim (origin:path == input_rel->cheapest_total_path)*/
			if (is_include_in_pathlist(path, input_rel->cheapest_cost_explore_pathlist)||
				pathkeys_contained_in(root->window_pathkeys, path->pathkeys))
				create_one_window_path(root,
									window_rel,
									path,
	...
	/* Now choose the best path(s), modified by jim */
	if(root->search_plan_mode==0){
		set_cheapest(window_rel);
	}else
	{
		/* Find and save the cheapest paths for this rel */
		set_cheapest_explore(window_rel);
	}
					   ...
					   List *activeWindows)
{
	...
	{
		add_path_explore(window_rel, path, root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
	}
}
					  ...
					  RelOptInfo *input_rel)
{
	...
	if (grouping_is_sortable(parse->distinctClause))
	{
		...
		foreach(lc, input_rel->pathlist)
		{
			...
			if (pathkeys_contained_in(needed_pathkeys, path->pathkeys))
			{
				...
				{
					add_path_explore(distinct_rel, (Path *)
						 create_upper_unique_path(root, distinct_rel,
												  path,
												  list_length(root->distinct_pathkeys),
												  numDistinctRows), root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
				}
			...
			list_length(root->sort_pathkeys))
		{
			...
			needed_pathkeys = root->distinct_pathkeys;
		/*modified by jim, origin: path = cheapest_input_path;*/
		if(root->search_plan_mode == 0){
		...
		{
			foreach(lc2, input_rel->cheapest_cost_explore_pathlist)
			{
	...
	if (allow_hash && grouping_is_hashable(parse->distinctClause))
	{
		...
		{
			foreach(lc3, input_rel->cheapest_cost_explore_pathlist)
			{
	...
	{
		/* Find and save the cheapest paths for this rel */
		set_cheapest_explore(distinct_rel);
	}
	...
	foreach(lc, input_rel->pathlist)
	{
		...
		}else
		{
			...
			if (is_include_in_pathlist(path, input_rel->cheapest_cost_explore_pathlist) || is_sorted)
			{
				...
				add_path_explore(ordered_rel, path, root->best_estimate_cost_of_current_query, root->prune_rate_for_add_path_explore);
			}
		}
...
/*
 * get_cheapest_explore_fractional_path
 *
 * We assume set_cheapest_explore() has been run on the given rel.
 * written by jim
 */
Path *
get_cheapest_explore_fractional_path(RelOptInfo *rel, double tuple_fraction, double rate)
{
	Path	   *best_path = rel->cheapest_total_path;
	ListCell   *l;
	Path       *best_explore_path;
    ListCell   *p;
	Cost       cost1,cost2;
	/* If all tuples will be retrieved, just return the cheapest-total path */
	if (tuple_fraction <= 0.0)
	{
		/* Find the best explore path satisfy our optimization purpose, modified by jim */
		best_explore_path = best_path;
		foreach(p, rel->pathlist)
		{
			Path *path = (Path *) lfirst(p);
			/* compare the cost requirement */
			if ((path->total_cost <= best_path->total_cost*(1+rate))&&(path->explore_value >= best_path->explore_value*(1+0)))
			//if (path->total_cost <= best_path->total_cost*(1+rate))
			{
				/* we will find best explore path */
				if(path->explore_value > best_explore_path->explore_value)
				{
					/* if larger than current best, replace it */
					best_explore_path = path;
				}
				else if (path->explore_value == best_explore_path->explore_value)
				{
					/* when the explore_value is same, we take cheaper cost's plan */
					if(path->total_cost < best_explore_path->total_cost){
						best_explore_path = path;
					}else if(path->total_cost == best_explore_path->total_cost){
						if(path->startup_cost < best_explore_path->startup_cost)
							best_explore_path = path;
					}else{
						/* nothing to do */
					}
		...
		return best_explore_path;
	}

	/* Convert absolute # of tuples to a fraction; no need to clamp to 0..1 */
	if (tuple_fraction >= 1.0 && best_path->rows > 0)
		tuple_fraction /= best_path->rows;
    /* When condsidering the start time, we need to try every plan in pathlist */
	/* First of all , we should find best_path*/
	foreach(l, rel->pathlist)
	{
		Path	   *path = (Path *) lfirst(l);
	...
	/* then find best explorevalue*/
	cost1 = best_path->startup_cost +
		tuple_fraction * (best_path->total_cost - best_path->startup_cost);
	...
	foreach(p, rel->pathlist)
	{
		...
		cost2 = path->startup_cost + tuple_fraction * (path->total_cost - path->startup_cost);
		/* compare the cost requirement */
		if ((cost2 <= cost1*(1+rate))&&(path->explore_value >= best_path->explore_value*(1+0)))
		//if (cost2 <= cost1*(1+rate))
		{
			/* we will find best explore path */
			if(path->explore_value > best_explore_path->explore_value)
			{
			...
			{
				/* when the explore_value is same, we take cheaper cost's plan */
				if(compare_fractional_path_costs(path, best_explore_path, tuple_fraction) < 0){
					best_explore_path = path;
				}else if(compare_fractional_path_costs(path, best_explore_path, tuple_fraction) == 0){
					if(path->total_cost < best_explore_path->total_cost){
						best_explore_path = path;
					}else if(path->total_cost == best_explore_path->total_cost){
						if(path->startup_cost < best_explore_path->startup_cost)
...
/**
 * another approach to choose current plan.
 * modified by jim 2021.3.11
 * */
Path *
get_cheapest_explore_fractional_path2(RelOptInfo *rel, double tuple_fraction, double rate1, Cost best_estimated_cost, double rate2)
{
	Path	   *best_path = rel->cheapest_total_path;
	ListCell   *l;
	Path       *best_explore_path;
	...
	if (tuple_fraction <= 0.0)
	{
		/* Find the best explore path satisfy our optimization purpose, modified by jim */
		best_explore_path = best_path;
		foreach(p, rel->cheapest_cost_explore_pathlist)
		{
			Path *path = (Path *) lfirst(p);
			/* compare the cost requirement */
			if (path->total_cost <= best_path->total_cost*(1+rate1))
			{
				if(path->total_cost <= best_estimated_cost*(1+rate2))
				{
					/* we will find best explore path */
					if(path->explore_value > best_explore_path->explore_value)
					{
...
/**
 * another approach to choose current plan.
 * modified by jim 2021.3.17
 * */
Path *
get_cheapest_explore_fractional_path3(RelOptInfo *rel, double tuple_fraction, double rate1, Cost best_estimated_cost, double rate2)
{
	Path	   *best_path = rel->cheapest_total_path;
	ListCell   *l;
	Path       *best_explore_path;
	...
	if (tuple_fraction <= 0.0)
	{
		/* Find the best explore path satisfy our optimization purpose, modified by jim */
		best_explore_path = best_path;
		final_cost = (best_path->total_cost*rate2 + best_estimated_cost*(1-rate2))*(1+rate1);
		foreach(p, rel->pathlist)
		//foreach(p, rel->cheapest_cost_explore_pathlist)
		{
			Path *path = (Path *) lfirst(p);
			/* compare the cost requirement */
			if ((path->total_cost <= final_cost)&&(path->explore_value >= best_path->explore_value*(1+0.5)))
			//if (path->total_cost <= final_cost)
			{
				/* we will find best explore path */
				if(path->explore_value > best_explore_path->explore_value)
				{
	...
	foreach(p, rel->pathlist)
	{
		...
		cost2 = path->startup_cost + tuple_fraction * (path->total_cost - path->startup_cost);
		/* compare the cost requirement */
		if ((cost2 <= cost1*(1+rate1))&&(path->explore_value >= best_path->explore_value*(1+0.5)))
		//if (cost2 <= cost1*(1+rate1))
		{
			/* we will find best explore path */
			if(path->explore_value > best_explore_path->explore_value)
					  ...
					  List *targets, List *targets_contain_srfs)
{
	...
	/* also apply to cheapest_total_explore_paths, written by jim */
	if (root->search_plan_mode > 1){
		foreach(lc, rel->cheapest_cost_explore_pathlist)
	    {
			Path	   *subpath = (Path *) lfirst(lc);
			Path	   *newpath = subpath;
			ListCell   *lc1,
